---
title: "05_results_assessprocess"
description: |
...
```{r}
#| label: fig-top3-disagr-types-conf-year
#| fig-cap: "Distribution of 3-top disagreement types per conference and year"
#| layout-ncol: 1
#| fig-column: page
...
disagr_top3 <-
  bind_rows(disagr_giscience_top3, disagr_agile_top3)

disagrTop3LinePlot <- function(data) {
  
  x_breaks = seq(min(data$year), max(data$year), by=1)
  y_breaks = seq(0.05, 0.7, by=.1)
  
  ggplot2::ggplot(data, 
                  aes(x = year, y = p, color = disagr_type)) +
  ggplot2::geom_line(linewidth = 1) +
  ggplot2::geom_point(size = 7.5) +
  ggplot2::geom_text(
    aes(label = scales::label_percent(accuracy = 1)(p)),
    vjust = 0.4,
    size = 3,
    color = "white",
    fontface = "bold") +
  ggplot2::facet_wrap(~conf, 
                      nrow = 1,
                      scales = "free_x") +
  ggplot2::scale_y_continuous(labels = scales::label_percent(), # Format y-axis as %
                              #breaks = seq(0,1,.1)*max(data$p)) +  
                              breaks = y_breaks)+   
  ggplot2::scale_x_continuous(breaks = x_breaks) +
  ggplot2::scale_color_brewer(type = "qual", palette = "Dark2") +
  ggplot2::guides(
      color = guide_legend(ncol = 3, position = "bottom")) +
  ggplot2::labs(
    x = "Year",
    y = "Proportion [%]",
    color = "Disagreement type"
...
fig_top3 <- disagrTop3LinePlot(disagr_top3)

fig_top3
```

```{r}
#| label: fig-top3-disagr-save
#| warning: false

#ggplot2::ggsave(file = here::here("figs", "GIScience_all.svg"),
#       width = 10, height = 7, dpi = 300)
ggplot2::ggsave(file = here::here("figs", "top3_disagr.png"),
...
disagr_uncertain <- 
  ...
  dplyr::mutate(disagr_dat = case_when(
    is.na(rev1_data) ~ TRUE,
    is.na(rev2_data) ~ TRUE,
    .default = (rev1_data != rev2_data))) |>
  dplyr::mutate(disagr_met = case_when(
    is.na(rev1_methods) ~ TRUE,
    is.na(rev2_methods) ~ TRUE,
    .default =(rev1_methods != rev2_methods))) |>
...
tbl_disagr_uncertain <-
  ...
  # Commented code for make table cells in bold. 
  #gt::tab_style(
  ...
  #gt::tab_options(footnotes.multiline = FALSE)  # All footnotes as a single line.
...
In @fig-disagr-uncertain-heatmap, we look now at the individual scores (UDAO levels) for each of the dimensions (data, methods, results), considering AGILE (top row) and GIScience (bottom row).

```{r}
#| label: fig-disagr-uncertain-heatmap
#| fig-cap: "Counts of UDAO scores for 'uncertain disagreement' between assessors (heatmap): AGILE vs GIScience"
#| fig-subcap: 
#|     - "AGILE: Data"
#|     - "AGILE: Methods"
#|     - "AGILE: Results"
...
tbl_disagr_significant <-
  ...
  gt::tab_footnote(
    footnote = md("Minor disagreements at the AGILE conference."),
    locations = cells_body(columns = p_res, rows = 2)
  ) |>
  gt::tab_footnote(
    footnote = md("Minor disagreements at the GIScience conference."),
    locations = cells_body(columns = c(p_met, p_res), rows = 4)
  )
...
@tbl-disagr-significant-conf corresponds to the **Table 5: Distribution of "significant disagreement" (N=42) per conference and criterion** of the published paper. 

What we observe in @tbl-disagr-significant-conf (only rows with `Disagr?` == `yes`) is: 
...
```{r}
#| label: table-disagr-significant-to-latex
#| eval: false
#| echo: false
