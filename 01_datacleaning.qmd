...
Reads original data assessment as a CSV file (`data` folder) and creates a CSV file (`data-clean` folder) called `all-data.csv`. This file includes all data items collected initially. Some transformations are applied.
...
#  dplyr::mutate(AGILE_badge = na_if(AGILE_badge, FALSE)) |> # No badge is codified as NA and FALSE -> consolidate to FALSE. ERROR HERE, replace by sentence below
  dplyr::mutate(AGILE_badge = if_else(is.na(AGILE_badge), FALSE, as.logical(AGILE_badge))) |> # No badge is codified as NA and FALSE.
...
- **significant disagreement**: when reviewers' notes (or lack of notes) cannot explain differences in assessment and conversation to get an agreement is necessary and/or checking again the paper is required. Initially codified as *major disagreement*; replaced by *significant disagreement*.
...
# "major disagreement" -> "significant disagreement"
all_data <- 
  all_data |>
  dplyr::mutate(disagr_type = if_else(disagr_type == "major disagreement", "significant disagreement", disagr_type))
...
all_data <- 
  ...
  dplyr::mutate(disagr_id = case_when(
    ...
    disagr_type == "significant disagreement" ~ 4))
