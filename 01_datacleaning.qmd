...
Reads original data assessment as a CSV file (`data` folder) and creates various CSV files (`data-clean` folder):

- `all-data.csv` file: all data items collected initially. Some transformations applied.
- `assessment.csv` file: data items required to explore feedback form reviewers during the assessment process. Individual scores (and notes) from two reviewers and types of disagreement between reviewers are relevant here
- `hypothesis.csv` file: data items required to explore [initial hyphotheses](https://doi.org/10.17605/OSF.IO/XZJCH). Consolidated scores are relevant here. 
...
all_data <- 
  ...
  dplyr::mutate(AGILE_badge = na_if(AGILE_badge, FALSE)) |> # No badge is codified as NA and FALSE -> consolidate to FALSE
...
Selected columns based on the `all-data` dataset.
