...
description: |
    Supporting notebook to compute the reproducibilty scores of the eligible papers. See the **Results** section in the final article.
...
Out of the `{r} n_papers`, **`{r} n_papers_conceptual`** were assigned as conceptual papers and they were  excluded from the reproducibility analysis. This represents
...
papers_giscience_data_pre |> 
  gt::gt() |>
  gt::tab_header(
    title = md("Distribution of UDAO levels in *Data* criterion"),
...
papers_giscience_results_pre |> 
  gt::gt() |>
  gt::tab_header(
    title = md("Distribution of UDAO levels in *Results* criterion"),
    subtitle = stringr::str_glue("2016 and 2018 GIScience papers (N={n_papers_giscience_pre})")
  ) |>
  gt::fmt_percent(
    columns = p,
...
plot1A <- criteriaBarplot2(papers_giscience_data_pre, col = consolidated_data, main = "Data", colour = colours[1], agg_level = T)

#plot2A <- criteriaBarplot(papers_giscience_pre, col = consolidated_methods, main = "Methods, analysis, processing", colour = colours[2])
plot2A <- criteriaBarplot2(papers_giscience_methods_pre, col = consolidated_methods, main = "Methods", colour = colours[2])
...
plot1B <- criteriaBarplot2(papers_giscience_data_post, col = consolidated_data, main = "Data", colour = colours[1], agg_level = T)

#plot2B <- criteriaBarplot(papers_giscience_post, col = consolidated_methods, main = "Methods, analysis, processing", colour = colours[2])
plot2B <- criteriaBarplot2(papers_giscience_methods_post, col = consolidated_methods, main = "Methods", colour = colours[2])
...
(plot1A + plot2A + plot3A) / (plot1B + plot2B + plot3B) + 
  plot_layout(widths = c(1,1,1)) + 
  plot_annotation(
    title = "GIScience paper assessment according to the criteria 'Data', 'Methods', and 'Results'.",
    subtitle = stringr::str_glue("Top: pre-guidelines (2016, 2018) papers (N={n_papers_giscience_pre}). Bottom: post-guidelines (2021 Part II, 2023) papers (N={n_papers_giscience_post}).\n X-axis labels: (U)ndocumented; (D)ocumented; Documented and (A)vailable; Documented, Available and (O)pen."),
...
plotA <- criteriaBarplot2(papers_agile_data_pre, col = consolidated_data, main = "Data", colour = colours[1], agg_level = T, agg_level_y = 45)

#plotB <- criteriaBarplot(papers_agile_pre, col = consolidated_methods, main = "Methods, analysis, processing", colour = colours[2])
plotB <- criteriaBarplot2(papers_agile_methods_pre, col = consolidated_methods, main = "Methods", colour = colours[2])
...
plotD <- criteriaBarplot2(papers_agile_data_post, col = consolidated_data, main = "Data", colour = colours[1], agg_level = T, agg_level_y = 45)

#plotE <- criteriaBarplot(papers_agile_post, col = consolidated_methods, main = "Methods, analysis, processing", colour = colours[2])
plotE <- criteriaBarplot2(papers_agile_methods_post, col = consolidated_methods, main = "Methods", colour = colours[2])
...
(plotA + plotB + plotC) / (plotD + plotE + plotF) + 
  plot_layout(widths = c(1,1,1)) + 
  plot_annotation(
    title = "AGILE paper assessment according to the criteria 'Data', 'Methods', and 'Results'.",
    subtitle = stringr::str_glue("Top: pre-intervention (2017-2019) papers (N={n_papers_agile_pre}). Bottom: post-intervention (2021-2024) papers (N={n_papers_agile_post}).\n X-axis labels: (U)ndocumented; (D)ocumented; Documented and (A)vailable; Documented, Available and (O)pen."),
...
# Same colors as per each criteria used before
colours <- RColorBrewer::brewer.pal(length(categoryColumns), "Set1")
#colours <- c("#E41A1C", "#E41A1C", "#377EB8", "#377EB8", "#4DAF4A", "#4DAF4A")

merged_giscience %>% 
  # To avoid redundancy, keep entries with HIGH level
  dplyr::filter(agg_level == "HIGH") |>
  ...
  ggplot2::labs(
    ...
    x = "Change in percentatge (gap) = % post - % pre", 
...
(plot_delta_giscience + plot_delta_agile)  + 
  ...
  plot_annotation(
    title = "GIScience vs AGILE: Change in percentage",
    subtitle = "Variation between HIGH and LOW before and after the intervention.")
...
#ggplot2::ggsave(file = here::here("figs", "GIScience_AGILE_delta.svg"),
#       width = 10, height = 7, dpi = 300)
...
data_tbl_absolute_change <- 
  bind_rows(
    merged_giscience |> 
      # To avoid redundancy, keep entries with HIGH level
      dplyr::filter(agg_level == "HIGH") |>
      dplyr::mutate(conf = "giscience") |>
      dplyr::select(conf, level, gap_p),
    
    merged_agile |>
      # To avoid redundancy, keep entries with HIGH level
      dplyr::filter(agg_level == "HIGH") |>
      dplyr::mutate(conf = "agile") |>
      dplyr::select(conf, level, gap_p)
  )
...
tbl_absolute_change <-
  ...
  tidyr::pivot_wider(names_from = conf, values_from = gap_p) |>
  gt::gt() |>
  gt::tab_header(
    title = "GIScience vs AGILE: Absolute change in percentatge points",
    subtitle = md("Variation between *HIGH* and *LOW* before and after the intervention.")
  ) |>
  gt::cols_label(
    level = md("Aggregate level per criterion*"),
    GISCIENCE = md("*GIScience*"),
    AGILE = md("*AGILE*"),
  ) |>
  gt::cols_align(
    align = c("center"),
...
```{r}
#| label: table-absolute_change-to-latex
#| eval: false
#| echo: false
