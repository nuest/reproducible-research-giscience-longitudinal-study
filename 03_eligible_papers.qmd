...
If we look at the distribution of conceptual papers per conference, we find that, `{r} scales::label_percent(accuracy = 0.1)(agile_conceptual$p)` (`{r} agile_conceptual$n` / `{r} n_papers_conceptual`) corresponds to AGILE conference papers, while the remaining  `{r} scales::label_percent(accuracy = 0.1)(giscience_conceptual$p)` corresponds to GIScience conference papers (`{r} giscience_conceptual$n` / `{r} n_papers_conceptual`). Since the number of AGILE papers (148) is almost double that of GIScience (78), there are no significant differences in the distribution of conceptual papers between conferences. However, a practical observation during the evaluation process is that theoretically oriented papers that included a short case study to demonstrate the value of the reported theory were more common at GIScience conferences. These types of papers were classified as nonconceptual, as input data, even small datasets for demonstration/validation purpose, was part of the study.
...
plot1 <- criteriaBarplot(papers_giscience, col = consolidated_data, main = "Data", colour = colours[1])
...
(plot1 + plot2 + plot3) + 
 ...
 plot_annotation(
    title = "GIScience paper assessment according to the dimensions Data, Methods and Results.",
...
#ggplot2::ggsave(file = here::here("figs", "GIScience_pre_post.svg"),
...
plotA <- criteriaBarplot(papers_agile_pre, col = consolidated_data, main = "Data", colour = colours[1])

plotB <- criteriaBarplot(papers_agile_pre, col = consolidated_methods, main = "Methods", colour = colours[2])
...
# "30" below is computed manually
breaks <- seq(from = 0, to = 30, by = 5)


plotD <- criteriaBarplot(papers_agile_post, col = consolidated_data, main = "Data", colour = colours[1])

plotE <- criteriaBarplot(papers_agile_post, col = consolidated_methods, main = "Methods", colour = colours[2])
...
Assessment results of the post-intervention AGILE papers (@fig-AGILE, bottom) present a completely different picture. To complete... 
...
(plotA + plotB + plotC) / (plotD + plotE + plotF) + 
  plot_layout(widths = c(1,1,1)) + 
  plot_annotation(
    title = "AGILE paper assessment according to the dimensions Data, Methods and Results.",
    subtitle = stringr::str_glue("Top: N={n_papers_agile_pre}. Pre-intervention papers (2017-2019); Bottom: N={n_papers_agile_post}. Post-intervention papers (2021-2024)."),
